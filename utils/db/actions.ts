...
import { NFTStorage, File } from "nft.storage"; // Import NFT.Storage SDK

console.log("ETHEREUM_RPC_URL:", process.env.ETHEREUM_RPC_URL);
console.log("PRIVATE_KEY:", process.env.PRIVATE_KEY);

let nftStorageClient: NFTStorage; // Declare NFTStorage client
let contract: ethers.Contract;

export async function initializeClients(userEmail: string) {
  console.log("+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++", "start")
  // Initialize the NFT.Storage client with your API key
  nftStorageClient = new NFTStorage({ token: process.env.NFT_STORAGE_API_KEY || "52fb3897.5c3d0fe426e7488ca00b3bf5c5f6fdbd"});
  console.log("===================================================================================================", nftStorageClient);
...
export async function uploadToNFTStorage(content: string, filename: string) {
  if (!nftStorageClient) {
...
export async function storeWebpageOnChain(domain: string, content: string) {
  console.log("firing NFT.Storage");
  if (!contract) {
    throw new Error("Contract not initialized");
  }
  const cid = await uploadToNFTStorage(content, "index.html");
  console.log("&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&", cid);

  const tx = await contract.storeWebpage(domain, cid);
  console.log("PPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPP", tx)
  await tx.wait();
...
export async function createWebpage(
  ...
  const deploymentUrl = `https://${cid}.ipfs.nftstorage.link/`;
...
async function updateNameContent(
  ...
  const cid = await uploadToNFTStorage(newContent, "index.html");
